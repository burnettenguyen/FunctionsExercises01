...
*/
function wordCharacterRetrieval(str) {
  let lastCharacter = "";
  let firstThreeCharacters = "";
  if (str.length <= 3) {
    lastCharacter = str.split("").reverse().join("");
    return lastCharacter[0];
  } else if (str.length > 3) {
    firstThreeCharacters = str.slice(0,3)
    return firstThreeCharacters;
  }
}
let str = "Functions rock!";
console.log(wordCharacterRetrieval(str));

console.log(`We put the '${wordCharacterRetrieval("Functions rock!")}' in '${str}'.`)
